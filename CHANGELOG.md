...
### Added
- Add an offline ZIP of the Claude Code lab pages (`ccw-hands-on-lab/ClaudeCode_Workshop_HandsOnLab.zip`, generated by `scripts/build-lab-zip.sh` in the deploy workflow and force-included in `_config.yml`) linked from every lab page's top-right toolbar and the workshop nav page
...
### Added
- Claude Code 랩 페이지 오프라인 ZIP 추가(`ccw-hands-on-lab/ClaudeCode_Workshop_HandsOnLab.zip`, 배포 워크플로에서 `scripts/build-lab-zip.sh`로 생성, `_config.yml`에 강제 포함) - 모든 랩 페이지 상단 우측 툴바와 워크샵 네비 페이지에서 링크
